                    ...
                    fn (idx entry)
                      [] idx $ div
                        {} (:tab-index 0)
                          :on-click $ fn (e d!)
                            on-select
                              conj parent-path $ &struct:get entry :key
                              , d!
                        div
                          {} $ :class-name style-child-entry
                          <> $ &struct:get entry :title
                          =< 8 nil
                          if
                            not $ empty? (&struct:get entry :children)
                            <> "|☰" $ {}
                              :color $ hsl 180 80 60
          :examples $ []
          :schema $ :: 'Fn
                  ...
                  comp-global-keydown
                    ...
                    fn (e d!)
                      cond
                          and
                            = |p $ &map:get e :key
                            or (&map:get e :meta?) (&map:get e :ctrl?)
                          .show quick-modal d!
                        (= |Escape (&map:get e :key))
                          .close quick-modal d!
                        true nil
                  .render quick-modal
              ...
              if (option:some? target)
                div
                  ...
                  div $ {} (:class-name css-markdown)
                    :innerHTML $ .!render md
                      &struct:get (option:unwrap target) :content
                  ; a $ {} (:inner-text |Speech)
                  ...
                  fn (idx path)
                    [] idx $ let
                      ...
                      div
                        ...
                        <> $ option:unwrap-or
                          option:map target $ fn (t) (&struct:get t :title)
                          , |
          :examples $ []
          :schema $ :: 'Dynamic
                      ...
                      div ({})
                        div
                          {} (:tab-index 0) (:class-name style-doc-entry)
                            ...
                            :on-click $ fn (e d!)
                              on-select
                                conj base-path $ &struct:get entry :key
                                , d!
                          <> $ &struct:get entry :title
                        if-let
                          xs $ &struct:get entry :children
                          div
                            {} $ :style
                              {} $ :padding-left 16
                            comp-nav-tree xs
              ...
              div
                ...
                list-> ({})
                  -> entries $ map-indexed
                    fn (idx entry)
                      [] idx $ let
                          selected? $ = selected (&struct:get entry :key)
                        div
                          {} (:tab-index 0)
                            :on-click $ fn (e d!)
                              on-select
